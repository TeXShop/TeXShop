...
%While the following document was originally written for distribution with the \TS\ Editor/Front End there are several other Editors/Front Ends that use a similar directive and the general discussion about File Encoding is valid no matter what Editor/Front End you use.
...
%Starting with \TS\ 4.00 the default, out of the box, encoding has become \cmd{UTF-8 Unicode}. If you have never set an encoding in \TS's \cmd{Preferences->Source} that is what will be used. If, at any previous time, you set that encoding it will not change from the one you have set.
