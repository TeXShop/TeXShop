...
Please read this document each time you upgrade; each release of TeXShop will contain a new version.

\section{Changes in 5.50 - 5.53}

No changes in version 5.51 - 5.53 require user intervention. Version 5.50 was never released.  

\section{Changes in 5.49}

Juan Luis Varona Malumbres, who maintains the Spanish localization of TeXShop, provided a new template for European users named the ``LatexSpanishTemplate''. The template includes packages highly recommended when writing in Spanish; comments in the template suggest related packages for other European languages. The new template is in $\sim$/Library/TeXShop/New. Drag the template from this location to the active folder $\sim$/Library/TeXShop/Templates if you will find it useful.



\section{Changes in 5.48}

A small error was found in the ``Close Begin/End'' macro. The word ``is'' occurs twice in line 127,
which reads ``tell me to to set environment\_name to do shell script''. To fix this, open the Macro editor,
select this macro, and remove the second ``to''. 

\section{Changes in 5.36 - 5.47}

No changes in version 5.36 require user intervention. Versions 5.37 - 5.41 were never released. No changes in version 5.42 require user intervention. Version 5.43 was not released. No changes in version 5.44 require user intervention.
Version 5.45 was not released. No changes in versions 5.46 - 5.47 require user intervention.

\section{Changes in 5.35}

This version contains a new magic comment line, and it is useful to create a macro which inserts this line. Since the macro is short, it is easiest to do this directly in the Macro editor. Go to the TeXShop Macros menu and select the first item ``Open Macro Editor..." An editing panel will appear showing existing macros on the left side. The first three items are likely to be ``Program, Encoding, Root.'' Select the item which will come just before the new macro. I selected ``Root''. Then below this list, push the button labeled ``New Item". The right side of the panel will switch to an area where the new macro can be created and named. Start at the top of this area by naming the item.
I named it ``Numbering''. In the Content area just below, type
\begin{verbatim}
  % !TEX numberingCorrection = 0 + current - desired
\end{verbatim}
This is the new macro. The text ``+ current - desired'' can be translated is desired, without hurting anything.
You are done editing, so push the ``Save'' button at the bottom of the edit panel.


\section{Changes in 5.29 - 5.34}

Version 5.29 was never released. No changes in versions 5.30 - 5.33 require user interaction. TeXShop 5.34 was never released.


\section{Changes in 5.28}

The release of version 5.28 coincides with the release of TeX Live 2024 and MacTeX-2024. Some of the subfolders of 
\path{~/Library/TeXShop/Engines/Inactive}, including {\bf Arara} and {\bf ConTeXt}, were revised for this release.
...
\section{Changes in 5.13 - 5.27}

No changes in versions 5.13 through 5.27 require user interaction.


\section{Changes in 5.12}

If you use PreTeXt, replace the active engines PreTeXt-LaTeX, PreTeXt-HTML, and PreTeXt-Both with the newer versions in \path{~/Library/TeXShop/Engines/Inactive/PreTeXt}.  

\section{Changes in 5.04 - 5.11}

No changes in versions 5.04, 5.08 - 5.12 require user interaction. Versions 5.05 through 5.07 were never released.
