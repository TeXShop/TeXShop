...
- (void) doTag: sender;
{
    ...
    while ((myRange.location < length) && (!done)) {
        ...
        if (start < length - 3) {
            ...
            else if ((theChar == 0x005c) && (start < length - 8) && (sectionIndex >= 0)) {
...
- (BOOL)textView:(NSTextView *)aTextView shouldChangeTextInRange:(NSRange)affectedCharRange replacementString:(NSString *)replacementString
{
    ...
    if ([SUD boolForKey: TagSectionsKey]) {
		...
		if (!tagLine) {
			...
			for(i = 0; i < [kTaggedTeXSections count]; ++i) {
				matchRange = [textString rangeOfString:[kTaggedTeXSections objectAtIndex:i] options:0 range:tagRange];
				if (matchRange.length != 0) {
					tagLine = YES;
					break;
				}
                ...
                if (theChar == 0x0025) 
                {
					...
					for(i = 0; i < [kTaggedTeXSections count]; ++i) {
						...
						if ([tagString isEqualToString:tag]) {
							...
							tagString = [NSString stringWithString:[kTaggedTagSections objectAtIndex:i]];
