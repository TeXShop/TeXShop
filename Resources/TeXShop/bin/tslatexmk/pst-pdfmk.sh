...
if test -z "$1.tex"; then
  ...
  cp "$1.tex" "$1-pics.tex"
#  cp "$1.aux" "$1-pics.aux"
  ${LTMKBIN}/latexmk -pdfdvi -ps- -dvi- -e '$latex = q/latex --shell-escape/; $dvipdf = q/dvips -o %B.ps %S ; ps2pdf14 -dAutoRotatePages=\/None %B.ps %D; pdfcrop %D ; \/bin\/mv %B-crop.pdf %D/' "$1-pics.tex"
