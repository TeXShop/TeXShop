...
\title{\TS\ Tips \& Tricks\\\small v0.8.2--2019/12/07}
\author{H. Schulz
%\raisebox{-5pt}{%
%\begin{tikzpicture}
%\begin{scope}[scale=0.2,yshift=-150pt]
%  \duck[body=yellow!50!brown!40!white,
%    crazyhair=gray!50!white,
%    eyebrow,
%    glasses=brown!70!black,
%%    book=\scalebox{0.1}{$E-mc^{2}$},
%    book=\scalebox{0.1}{\includegraphics[scale=0.05]{figs/TeXShop.png}},
...
It is possible to make a few changes and additions to the \LaTeX\ Panel by editing the \path{~/Library/TeXShop/LatexPanel/completion.plist} file. \textbf{Note: all \cmd{plist} files must be edited using UTF-8 Unicode encoding.}

\subsubsection{The Tags and Labels Popups}
...
Like the \mnu{Tags} popup menu the \mnu{Labels} popup, available in \TS\ 4.18 and later, automatically lists labels, rather than tags, in the document you're document. Similar to the \mnu{Tags} popup it only contains Labels for the edited document. Choosing one of those Labels from the list takes you to the corresponding \verb|\Label| command.
...
\paragraph{\cmd{Apple Spell Checker}}

The main problem with using the \cmd{Apple Spell Checker} with \TeX\ is that the normal dictionaries will flag most \TeX\ and \LaTeX\ command names and many arguments of those commands which is rather annoying. \TS\ 4.18 and later have introduced some help to solve some of those problems as long as you check \mnu{Edit}\To\mnu{Check Spelling While Typing} and use one of the Apple Dictionaries (i.e., not ones used for \cmd{cocoAspell}---see below).You need to check one or more of the items in the \cmd{Spell Checking} section of the \cmd{Source} tab of \TS\To\mnu{Preferences}. The most important item to check is the \cmd{Do not check TeX commands}. Please read the \cmd{TeXShop Changes 4.18} section of the \mnu{Help}\To\mnu{Changes} document for more detailed information.
...
If you have changed the syntax colors for pre-4.08 versions of \TS\ using the method given above those colors will \emph{not} be used by version 4.08 out of the box. To retrieve those colors see the method in the \mnu{Help}\To\mnu{Changes} document.
\begin{figure}
\centering
\subfloat[][]{%
...
bf               & ;            & midr       & midrule        & rnewcoo         & renewcommand \\
...
%\section{---\quad \TS\ Directives}\label{sec:Directives}
%
